...
methods_code <- c("gs", "stan", "bpr") # Assuming "bpr0" is your BPR method

# Corresponding labels for plotting
plot_method_labels <- c("GS", "BPR", "Stan") # Renamed BPR0 to BPR for clarity
...
plot_densities <- ggplot(density_data, aes(x = x, y = y, color = method, linetype = method)) +
  ...
  scale_linetype_manual(values = c("GS" = "dashed", "BPR" = "dashed", "Stan" = "solid")) +
